...
Nesse desafio foi desenvolvido uma aplicação de gestão de transações para treinar o que foi aprendido até este momento no Node.js junto ao TypeScript, mas dessa vez incluindo o uso de banco de dados com o TypeORM e envio de arquivos com o Multer!

É aplicação que deve armazenar transações financeiras de entrada e saída e permitir o cadastro e a listagem dessas transações, além de permitir a criação de novos registros no banco de dados a partir do envio de um arquivo csv.
...
**Dica**: Antes de criar uma nova categoria, sempre verifique se já existe uma categoria com o mesmo título. Caso ela exista, use o `id` já existente no banco de dados.
...
**Dica 2**: Crie os bancos de dados solicitados (gostack_desafio06_tests, gostack_desafio06). Foi usado o postgres.

```bash
# Clone this repository
$ git clone https://github.com/marciofrancalima/desafio-database-upload.git (or use ssh)
